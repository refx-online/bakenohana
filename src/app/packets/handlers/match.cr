...
class MatchChangeSlotPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
...
class MatchReadyPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchNotReadyPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchLockPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
    @slot_id = @reader.read_i32
  end
...
class MatchChangeSettingsPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
    @data = @reader.read_match
  end
...
class MatchStartPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
    return unless p.same?(m.host)
...
class MatchScoreUpdatePacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
...
class MatchCompletePacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchChangeModsPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
...
class MatchLoadCompletePacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchNoBeatmapPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchHasBeatmapPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchFailedPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchSkipRequestPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
...
class MatchTransferHostPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
    @slot_id = @reader.read_i32
  end
...
class MatchInvitePacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
...
class MatchChangePasswordPacket < BasePacket
  def initialize(@reader : BanchoPacketReader)
    @data = @reader.read_match
  end
...
class MatchChangeTeamPacket < BasePacket
  def handle(p : Player)
    m = p.match
    return unless m
